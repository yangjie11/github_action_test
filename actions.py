...
def main():
    ...
    # os.system("curl -o {0} {1} > /dev/null 2>&1".format(ZIP_NAME, CHROMEDRIVER))
    # os.system("unzip {0} -d /usr/local/bin > /dev/null 2>&1".format(ZIP_NAME))
    # os.system("ls -al /usr/local/bin")
    # os.system("rm -rf {0}".format(ZIP_NAME))
    ...
    logging.info("chromedriver verison : ")
    # os.system("chromedriver")
    # os.system("chrome")
    logging.info("ready to sign in")
